...
from .utils import ExchangeOperator
...
class Solution(object):
    ...
    def __init__(self, vehicle_id_to_planned_route):
        ...
        self.total_cost = 99999999  # 代价 = 总超时小时数*10000 + 车辆平均km (时间秒距离/120)
        ...
        for vid, route in self.vehicle_id_to_planned_route.items():
            ...
            for idx in range(len(route)):
                if route[idx].order_index < 0:  # 送货节点
                    ...
                    self.over_time[vid] += max(0, t_finish - t_commit)

        self.total_over_time = sum(self.over_time.values())
...
def first_solution(id_to_unallocated_order_item: dict, id_to_vehicle: dict, id_to_factory: dict):
    ...
    for item_id, item in id_to_unallocated_order_item.items():  # 遍历每个货物
        ...
        if order_id in id_to_all_myorder:
            continue
    ...
    for order_id, items in order_id_to_items.items():  # 依次查看决策每个订单
        ...
        if id_to_all_myorder[order_id].demand > MAX_CAPACITY:
            ...
            while item_idx < len(items):
                ...
                for vehicle_id, vehicle in id_to_vehicle.items():
                    ...
                    t_delivery = pickup_node.service_time + node_matrix[pickup_node_int_id][delivery_node_int_id] \
                                 + DOCK_TIME*2 + delivery_node.service_time  # 取送货整个完成时间
                    # 预估超时时间
                    # last_leave_time = temp_solution.time_window[vehicle_id][-1][1]  # 路径最后一个点的离开时间
            ...
            if insert_pos == -1:
                ...
                vehicle_id_to_planned_route[min_vid].insert(insert_pos, pickup_node)
                vehicle_id_to_planned_route[min_vid].insert(insert_pos + 1, delivery_node)

    # 搜索优化当前初始解
    solution = Solution(copy.deepcopy(vehicle_id_to_planned_route))  # 临时解
    solution.init_time_window(id_to_vehicle)
    solution.calc_cost(update=True, id_to_vehicle=id_to_vehicle)
    vehicle_id_to_planned_route = optimize_solution(solution, id_to_vehicle)
